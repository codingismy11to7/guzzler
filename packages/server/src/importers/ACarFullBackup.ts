...
import { BigDecimal, Data, Effect, Match, Option, pipe, Schema, Stream, Struct } from "effect";
...
const ACarFuelType = Schema.Struct({
  ...
  name: String,
  notes: OptionalString,
  ratingType: Schema.OptionFromUndefinedOr(Schema.Literal("octane_ron", "octane_aki", "cetane")),
...
const parsePlaceFields = (fields: typeof PlaceFieldsSchema.Type) =>
  gen(function* () {
    const { deviceLatitude, deviceLongitude, placeLatitude, placeLongitude, ...placeFields } = fields;

    const makeLocation = (lat: Option.Option<Latitude>, long: Option.Option<Longitude>) =>
      Option.gen(function* () {
    ...
    const deviceLocation = encodeLocationOpt(makeLocation(deviceLatitude, deviceLongitude));
    const placeLocation = makeLocation(placeLatitude, placeLongitude);

    const removePlace = (s: string) => s.replace(/place[A-Z]/, a => a.slice(-1).toLowerCase());

    const place =
      Option.isNone(placeLocation) && Object.values(placeFields).every(isUndefined)
        ? undefined
        : yield* Schema.encode(Place, { onExcessProperty: "error" })({
            location: placeLocation,
            ...Object.fromEntries(Object.entries(placeFields).map(([k, v]) => [removePlace(k), v])),
          });

    return { place, deviceLocation };
  });

const child = (n: Xml2JsNode, name: string) => n.children.find(n => n.name === name);
const childObjects = (n: Xml2JsNode, ...names: string[]) =>
  names.reduce((acc, name) => ({ ...acc, [name]: (child(n, name)?.children ?? []).map(childrenAsObj) }), {});
const childrenAsObj = (n: Xml2JsNode): Record<string, unknown> =>
  Object.fromEntries(n.children.map(child => [child.name, child.children.length ? childrenAsObj(child) : child.text]));

class UnexpectedOpeningTag extends Data.TaggedError("UnexpectedOpeningTag")<{ expected: string; received: string }> {
  get message() {
    return `Expected an opening XML tag of '${this.expected}', but received '${this.received}'`;
  }
}
const unexpectedOpeningTag = (expected: string, received: string) => new UnexpectedOpeningTag({ expected, received });

class MissingBackupFile extends Data.TaggedError("MissingBackupFile")<{ fileName: string }> {
  get message() {
...
const importFromACarFullBackup =
  ...
  (
    ...
    gen(function* () {
      ...
      ): Effect.Effect<readonly EventSubtype[], ParseError | XmlParsingError | UnexpectedOpeningTag | E> =>
        Effect.gen(function* () {
          const doc = yield* parseEntireXml(data);

          if (doc.name !== "eventSubtypes") yield* unexpectedOpeningTag("eventSubtypes", doc.name);

          const convert = (n: Xml2JsNode): Effect.Effect<ACarEventSubtype, UnexpectedOpeningTag | ParseError> =>
            n.name !== "eventSubtype"
          ...
          const parsed = yield* Effect.forEach(doc.children, convert, { concurrency: "unbounded" });
          ...
          return yield* Effect.forEach(parsed, convertToDataModel, { concurrency: "unbounded" }).pipe(
      ...
      const parseFuelTypes = <E>(
        data: Stream.Stream<Uint8Array, E>,
      ): Effect.Effect<readonly FuelType[], ParseError | XmlParsingError | UnexpectedOpeningTag | E> =>
        Effect.gen(function* () {
          const doc = yield* parseEntireXml(data);

          if (doc.name !== "fuelTypes") yield* unexpectedOpeningTag("fuelTypes", doc.name);

          const convert = (n: Xml2JsNode): Effect.Effect<ACarFuelType, UnexpectedOpeningTag | ParseError> =>
            n.name !== "fuelType"
              ? unexpectedOpeningTag("fuelType", n.name)
              : pipe(
                  { id: n.attrs.id, ...childrenAsObj(n) } as Record<string, unknown>,
      ...
      const parseTripTypes = <E>(
        data: Stream.Stream<Uint8Array, E>,
      ): Effect.Effect<readonly TripType[], ParseError | XmlParsingError | UnexpectedOpeningTag | E> =>
        Effect.gen(function* () {
          const doc = yield* parseEntireXml(data);

          if (doc.name !== "tripTypes") yield* unexpectedOpeningTag("tripTypes", doc.name);

          const convert = (n: Xml2JsNode): Effect.Effect<ACarTripType, UnexpectedOpeningTag | ParseError> =>
            n.name !== "tripType"
              ? unexpectedOpeningTag("tripType", n.name)
              : pipe({ id: n.attrs.id, ...childrenAsObj(n) }, Schema.decodeUnknown(ACarTripType));

          const parsed = yield* Effect.forEach(doc.children, convert);

          const convertToDataModel = (tt: ACarTripType) => Schema.decode(TripType, { onExcessProperty: "error" })(tt);
      ...
      const parseVehicles = <E>(
        data: Stream.Stream<Uint8Array, E>,
      ): Effect.Effect<void, XmlParsingError | ParseError | MongoError | NotFound | UnexpectedOpeningTag | E> =>
        Effect.gen(function* () {
          ...
          if (currEvt._tag !== "StartElement" || currEvt.name !== "vehicles")
            ...
            yield* unexpectedOpeningTag("vehicles", currEvt._tag === "StartElement" ? currEvt.name : `${currEvt}`);

          const parseAndHandleNextVehicle: Effect.Effect<
            void,
            Option.Option<XmlParsingError | ParseError | MongoError | NotFound | E>
          > = gen(function* () {
            const nextIsVehicleStart = pipe(
              peekNext,
              Effect.andThen(e => e._tag === "StartElement" && e.name === "vehicle"),
            );
            ...
            const vehicleStream = Stream.unfoldEffect(false, seenEnd =>
              seenEnd
                ...
                : pipe(
                    pullNext,
                    Effect.andThen(evt =>
                      Effect.succeed<Option.Option<readonly [ParseEvent, boolean]>>(
                        Option.some([evt, evt._tag === "EndElement" && evt.name === "vehicle"]),
                      ),
                    ),
                  ),
            );
            ...
            const node = yield* transformToJson(vehicleStream).pipe(
              Effect.catchTag("XmlParsingError", e => Effect.fail(Option.some(e))),
            );
            ...
            yield* controller.withPausedStream(
              gen(function* () {
                const aCarVehicle = yield* Schema.decodeUnknown(ACarVehicle)({
                  id: node.attrs.id,
                  ...childrenAsObj(node),
                  ...childObjects(node, "vehicleParts", "reminders", "tripRecords"),
                  fillupRecords: (child(node, "fillupRecords")?.children ?? []).map(n => ({
                    id: n.attrs.id,
                    ...childrenAsObj(n),
                  })),
                  eventRecords: (child(node, "eventRecords")?.children ?? []).map(n => ({
                    id: n.attrs.id,
                    ...childrenAsObj(n),
                  })),
                ...
                const { id, photo, fuelTankCapacity, fillupRecords, eventRecords, ...rest } = aCarVehicle;

                const vehicle = yield* Schema.decode(Vehicle, { onExcessProperty: "error" })({
                  id: VehicleId.make(id),
                  ...rest,
                  photoId: undefined,
                  fuelTankCapacity: fuelTankCapacity.pipe(Option.map(BigDecimal.format), Option.getOrUndefined),
                }).pipe(Effect.asSomeError);
                ...
                if (Option.isSome(photo)) {
                  const fType = yield* Effect.promise(() => fileTypeFromBuffer(photo.value));
                ...
                const convertFillup = (fr: ACarFillupRecord) =>
                  gen(function* () {
                    const addlFields = ["partial", "previousMissedFillups", "hasFuelAdditive"] as const;
                    ...
                    const timestamp = parseDate(date, "MM/dd/yyyy - HH:mm", new Date(), {
                      in: tz(userTimeZone),
                    }).getTime();

                    // wrote the parse place fields code to want the decoded version
                    const placeFields = Struct.pick(fr, ...placeFieldNames);
                    const { place, deviceLocation } = yield* parsePlaceFields(placeFields);

                    const addl = Struct.pick(fr, ...addlFields);

                    return yield* Schema.decode(FillupRecord, { onExcessProperty: "error" })({
                ...
                const fillups = yield* Effect.forEach(fillupRecords, convertFillup, {
                ...
                const convertEvent = (er: ACarEventRecord) =>
                  gen(function* () {
                    ...
                    return yield* Schema.decode(EventRecord, { onExcessProperty: "error" })({
                ...
                const events = yield* Effect.forEach(eventRecords, convertEvent, { concurrency: "unbounded" });
                yield* autos.insertEventRecords(username, vehicle.id, events);
                yield* Effect.logInfo(`Added ${eventRecords.length} event records`);
              }),
            );
          });

          while (isNotUndefined(yield* peekNext)) yield* parseAndHandleNextVehicle;
        }).pipe(
          Effect.catchTags({ None: () => Effect.void, Some: e => Effect.fail(e.value) }),
      ...
      const openBackupFile = (fileName: string): Stream.Stream<Uint8Array, MissingBackupFile | SystemError> =>
        Stream.unwrap(
          gen(function* () {
            const targetFile = yield* Effect.fromNullable(
              backupFiles.filter(e => !e.isDirectory).find(e => path.basename(e.fileName) === fileName),
            ).pipe(catchTag("NoSuchElementException", () => new MissingBackupFile({ fileName })));

            return fs.stream(targetFile.fileName).pipe(Stream.catchTags({ BadArgument: Stream.die }));
          }),
        );

      const eventSubtypes = yield* parseEventSubtypes(openBackupFile("event-subtypes.xml"));
      ...
      yield* Effect.logInfo("Parsed settings, going to start writing to the database");
      ...
      yield* inTransactionRaw()(
        gen(function* () {
          ...
          yield* autos.replaceAllUserTypes(
            UserTypes.make({
              username,
              eventSubtypes: Object.fromEntries(eventSubtypes.map(e => [e.id, e])),
      ...
      catchTags({
        UnexpectedOpeningTag: () => new AutosApi.WrongFormatError({ type: "UnexpectedOpeningTag" }),
        MissingBackupFile: () => new AutosApi.WrongFormatError({ type: "MissingBackupFile" }),
        ParseError: () => new AutosApi.WrongFormatError({ type: "ParseError" }),
        ZipError: () => new AutosApi.FileCorruptedError({ type: "ZipError" }),
        XmlParsingError: () => new AutosApi.FileCorruptedError({ type: "XmlParsingError" }),
...
export class ACarFullBackup extends Effect.Service<ACarFullBackup>()("ACarFullBackup", {
  accessors: true,
  effect: gen(function* () {
