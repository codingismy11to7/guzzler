...
import { AuthRedirectMiddleware, RawSessionAccess_DoNotUse } from "./Authentication.js";
import { RedactedError, ServerError } from "./Errors.js";

class UI extends HttpApiGroup.make("ui")
  .add(HttpApiEndpoint.get("ui", "/*").addError(NotFound).addError(RedactedError).addError(ServerError))
...
export class AppApi extends HttpApi.make("Guzzler")
  ...
  .add(TodosApiGroup)
  .add(UI)
  .annotateContext(OpenApi.annotations({ title: "Guzzler", version: "current" })) {}
